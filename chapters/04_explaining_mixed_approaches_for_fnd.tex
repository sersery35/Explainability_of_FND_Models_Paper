...
\begin{table}
    ...
    \begin{tabular}{c | c | c | c | c}
        ...
        \hline
        UPFD-Gossipcop   & 95.73\%           & 95.81\%            & 95.70\%         & 95.71\%           \\
        \hline
        UPFD-Politifact  & 82.44\%           & 82.95\%            & 82.50\%         & 82.32\%           \\
    \end{tabular}
    \caption[The UPFD classifier performance metrics averaged over 10 runs.]{The UPFD classifier macro averaged performance metrics averaged over 10 runs.}
...
\end{figure}
We examine whether our classifier learned a similar intuition that we discussed in~\ref{subsec:fakeNewsDetection_fakeNews}. Recall that echo chambers are one of the significant reasons that fake news pieces spread. We analyze an example from the train split of the UPFD-Politifact dataset, some of whose cascade nodes exhibit echo chamber structures. To be precise, there is no clear definition of echo chambers in the context of diffusion trees. For the echo example in Fig.~\ref{fig:echoChamberExample1}, we assume that any cascade node with more than one child node, each of which do not have children, has an echo chamber. This assumption is based on a couple of properties listed below.
\begin{enumerate}
    \item The diffusion tree is made in such a way that the retweet relationship between two users are based on the timestamp and visibility of the retweet, and follower/followee relationship as discussed in~\ref{subsec:mixedApproaches_DatasetAndModel}. Therefore, if the tweet is fake, it is likely that no one else except the ones in the echo chamber will retweet that news piece.
    \item The users in an echo chamber are likely to follow like-minded people which leads to the consumption of retweets from the same user. This is also enforced by the recommendation algorithms in social media as discussed in~\ref{subsec:fakeNewsDetection_FoundationsOfFakeNews}.
    \item The lower bound for the number of users is set to the lowest possible to capture all potential echo chambers. One can increase the lower bound to capture bigger echo chambers and work with them.
\end{enumerate}
We make this assumption to discover the explainability of the UPFD classifier by a concrete example.\\
...
\begin{table}
    ...
    \end{tabular}
    \caption[Model's fake news prediction probabilities for input perturbation experiment.]{Model's fake news prediction probabilities for input perturbation experiment. $p_{original}$ denotes fake news prediction probability for the original input. Analogously, $p_{subgraph}$ refers to the fake news prediction probability for the explaining subgraph.}
